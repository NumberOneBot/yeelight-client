...
class LightChannel {
  ...
  setRGB(r: number, g: number, b: number, opts?: TransitionOptions): Promise<void>
  setHSV(hue: number, saturation: number, opts?: TransitionOptions): Promise<void>
  ...
  setAdjust(action: 'increase' | 'decrease' | 'circle', prop: 'bright' | 'ct'): Promise<void>
