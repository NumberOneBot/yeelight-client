...
class YeelightDevice {
  ...
  getState(): Promise<{ main: ChannelState; bg: ChannelState | null }>
  // Fetches current state of all channels in a single get_prop request.
  // Preferred over per-channel polling — avoids an extra round-trip.
...
> **Protocol limits:** maximum **9 frames** per flow (the device returns an error for more); minimum frame duration **50 ms** (the device clamps shorter values).
