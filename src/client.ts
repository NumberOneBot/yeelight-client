...
export class YeelightClient {
  ...
  connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      this.socket = net.createConnection({ host: this.device.ip, port: this.device.port }, resolve);
  ...
  setSegments(leftR: number, leftG: number, leftB: number, rightR: number, rightG: number, rightB: number) {
  ...
  setAll(r: number, g: number, b: number, effect: 'smooth' | 'sudden' = 'smooth', duration = 500) {
    const rgb = (r << 16) | (g << 8) | b;
    return this.send('bg_set_rgb', [rgb, effect, duration]);
  }

  setPower(on: boolean, effect: 'smooth' | 'sudden' = 'smooth', duration = 500) {
    return this.send('bg_set_power', [on ? 'on' : 'off', effect, duration]);
  }

  setBrightness(brightness: number, effect: 'smooth' | 'sudden' = 'smooth', duration = 500) {
    return this.send('bg_set_bright', [Math.max(1, Math.min(100, brightness)), effect, duration]);
  }
